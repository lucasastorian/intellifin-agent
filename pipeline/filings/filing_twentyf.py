...
class FilingTwentyF(BaseFiling):
    ...
    async def upsert(self):
        ...
        section_chunks_task = self._upsert_filing_section_chunks(sections=sections, filing=filing)
        ...
        filing_note_task = self._upsert_filing_note_chunks(note_ids=note_ids, processed_notes=processed_notes, filing=filing)

        # TODO: Attachments / press release??
        attachment_task = self._upsert_attachments_and_pages(filing_id=filing['id'])

        _, _, attachment_data = await asyncio.gather(section_chunks_task, filing_note_task, attachment_task)
